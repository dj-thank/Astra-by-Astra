...
ContactEvent FlightSimulation::Step(const FlightInput& raw) {
    ...
    if(localCruise&&flightAssistEnabled_) {
        ...
        if(nearby) {
            ...
            const double alignment=Vec3d::Dot(from,to);
            if(axis.Length()>1e-12) {
                const auto transport=Quatd::FromAxisAngle(axis,std::atan2(axis.Length(),alignment));
                ...
                // Antipodal step: cross product vanishes but the horizon flipped.
                // Rotate 180 degrees around any perpendicular axis.
                Vec3d fallback=Vec3d::Cross(from,Vec3d{0,0,1});
                if(fallback.Length()<1e-6) fallback=Vec3d::Cross(from,Vec3d{0,1,0});
                const auto transport=Quatd::FromAxisAngle(fallback.Normalized(),Pi);
                state_.orientation=(transport*state_.orientation).Normalized();
                state_.velocityMetersPerSecond=transport.Rotate(state_.velocityMetersPerSecond);
            }
        }
