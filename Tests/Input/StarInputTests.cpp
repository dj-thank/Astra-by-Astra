...
int main() {
    ...
    for (int i = 0; i < 5; ++i) safety.Update(raw,p,true,true,0.1f);
    EXPECT(safety.IsArmed());
    ThrottleInputArbiter throttle;
    EXPECT(Near(throttle.Resolve(0.0f, 0.0f, true, false), 0.0f));
    EXPECT(Near(throttle.Resolve(0.4f, 0.0f, true, true), 0.4f));
    EXPECT(Near(throttle.Resolve(0.4f, 0.0f, true, false), 0.4f));
    EXPECT(Near(throttle.Resolve(0.4f, 0.2f, true, false), 0.4f));
    EXPECT(Near(throttle.Resolve(0.4f, 0.39f, true, false), 0.39f));
    EXPECT(Near(throttle.Resolve(0.7f, 0.6f, true, true), 0.7f));
    EXPECT(Near(throttle.Resolve(0.7f, 0.6f, true, false), 0.7f));
    EXPECT(Near(throttle.Resolve(0.7f, 0.8f, true, false), 0.8f));
    EXPECT(Near(throttle.Resolve(0.7f, 0.8f, false, false), 0.7f));
    ThrottleInputArbiter delayedController;
    EXPECT(Near(delayedController.Resolve(0.5f, 0.0f, false, true), 0.5f));
    EXPECT(Near(delayedController.Resolve(0.5f, 0.0f, false, false), 0.5f));
    EXPECT(Near(delayedController.Resolve(0.5f, 0.1f, true, false), 0.5f));
    EXPECT(Near(delayedController.Resolve(0.5f, 0.5f, true, false), 0.5f));
