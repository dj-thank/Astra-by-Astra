...
ART=ROOT/'work/cockpit-v04'
CONTEXT=Path(os.environ.get('STAR_CONTEXT_ROOT', str(ROOT)))
assert (CONTEXT/'Art/Explorer/V2/STAR_Explorer_V2.blend').exists(), 'Set STAR_CONTEXT_ROOT to the canonical read-only context checkout'
...
exec(compile((Path(__file__).parent/'v04_manufacturing.py').read_text(encoding='utf-8'), 'v04_manufacturing.py', 'exec'))
...
box('Glare shield aluminum core',(7.15,0,.979),(.61,2.68,.022),dark,.003)
box('Glare shield textured overmould',(7.14,0,.996),(.59,2.67,.012),polymer,.005)
...
for y in (-.88,0,.88):
 ...
 for z in (.54,.943):
  box('Display housing folded return',(6.885,y,z),(.046,.786,.046),dark,.002)
  box('Display bezel machined front',(6.857,y,z),(.031,.78,.041),dark,.002)
  box('Display front engraved legend strip',(6.840,y,z),(.002,.684,.019),phenolic,.0005)
 for sy in (-1,1):
  box('Display side housing seam',(6.889,y+sy*.369,.743),(.047,.047,.365),rubber,.001)
  box('Display bezel vertical',(6.860,y+sy*.369,.743),(.032,.042,.365),dark,.0018)
  box('Display outer fastening flange',(6.902,y+sy*.397,.743),(.006,.023,.372),dark,.0015)
  for zz in (.582,.908):fastener((6.897,y+sy*.397,zz),'X')
 ...
 for i in range(9):
  ...
  box('Tactile soft key',(6.814,yy,.549),(.012,.036,.014),phenolic,.0012)
 ...
 # Thin folded louvres with true open slots and supported flanges.
 vent_louvres(y)
 ...
 box('Control strip folded housing',(7.34,y,1.055),(.12,.49,.10),dark,.002)
 box('Control strip removable face',(7.276,y,1.055),(.005,.48,.089),phenolic,.001)
 for yy in (y-.232,y+.232):box('Control strip folded flange',(7.336,yy,1.006),(.137,.018,.003),dark,.001)
 box('Control strip rear gasket',(7.274,y,1.055),(.007,.464,.079),rubber,.002)
 for idx in range(5):
  yy=y-.176+idx*.088
  box('Recessed control switch body',(7.268,yy,1.063),(.014,.068,.054),dark,.0015)
  box('Switch inset retaining frame',(7.259,yy,1.063),(.002,.061,.047),alloy,.0006)
  box('Recessed control cap',(7.255,yy,1.063),(.007,.057,.043),phenolic,.0015)
...
# Narrow replaceable system plates occupy the support webs between MFDs.
for yy in (-.44,.44):
 box('Avionics interbay sealing strip',(6.932,yy,.746),(.009,.078,.416),rubber,.002)
 box('Interbay equipment face',(6.923,yy,.746),(.006,.068,.397),dark,.0015)
 for zz in (.573,.918):fastener((6.918,yy,zz),'X')
 for zz,legend in ((.674,'PWR'),(.777,'BUS')):
  cyl('Panel push-pull circuit breaker collar',(6.918,yy,zz),(6.906,yy,zz),.0105,alloy,20)
  cyl('Panel circuit breaker cap',(6.906,yy,zz),(6.893,yy,zz),.0075,phenolic,20)
  text('Circuit breaker engraved legend',legend,(6.918,yy,zz+.025),.008,rear)
...
for side in (-1,1):
 ...
 for i,x in enumerate((5.58,6.17,6.76,7.35)):
  ...
  liner_panel(x,wall_y,height,side)
  for xx in (x-.23,x+.23):
...
for name,objects in parts.items():
 ...
 # FBX tangent exporter requires only triangles/quads; bake triangulation after weighted normals.
 tri=o.modifiers.new('Export triangulation with weighted normals','TRIANGULATE')
 ...
 bpy.ops.export_scene.fbx(filepath=str(OUT/'Parts'/f'{name}.fbx'),use_selection=True,axis_forward='X',axis_up='Z',global_scale=1,apply_unit_scale=True,apply_scale_options='FBX_SCALE_UNITS',object_types={'MESH'},bake_anim=False,path_mode='RELATIVE',use_mesh_modifiers=True,use_tspace=True)
 final.append(o)
bpy.ops.object.select_all(action='DESELECT')
...
metadata={'version':4,'status':'candidate_not_game_verified','source_units':'meters','source_frame':{'forward':'+X','right':'+Y','up':'+Z'},'fbx_axis_forward':'X','fbx_axis_up':'Z','fbx_apply_unit_scale':True,'fbx_scale_options':'FBX_SCALE_UNITS','import_uniform_scale':1,'model_parts':manifest,'materials':specs,'replace':{'SM_CockpitInterior':'SM_CockpitV3Interior','SM_InstrumentPanel':'SM_CockpitV3InstrumentPanel'},'retain':['SM_CockpitShell','SM_CanopyGlass','SM_ControlStick','SM_Throttle'],'camera_m':[6,0,1.3],'display_sockets':[{'location_m':[6.838,y,.747],'normal':[-1,0,0],'size_m':[.619,.312]} for y in (-.88,0,.88)],'glass_contract':{'base_color_linear':[.995,.995,.995],'opacity':0,'two_sided':False,'action':'do not edit glass material'},'new_geometry_original':True,'scientific_observation':False}
(ART/'manifest.json').write_text(json.dumps(metadata,ensure_ascii=False,indent=2),encoding='utf-8')
(OUT/'manifest_v04.json').write_text(json.dumps(metadata,ensure_ascii=False,indent=2),encoding='utf-8')

# Materials and triangle budgets are versioned in owned asset metadata.
metadata['manufacturing_changes'] = MANUFACTURING_CHANGES
metadata['baseline_triangles'] = {'SM_CockpitV3InstrumentPanel':127296, 'SM_CockpitV3Interior':267892}
metadata['ue_tangent_strategy']='Import authored normals; remove degenerate triangles; recompute tangents with MikkTSpace'
metadata['fbx_tangent_export']=True
metadata['triangulated_before_export']=True
metadata['normal_convention']='OpenGL tangent-space +Y; UE texture importer must flip green once'
metadata['material_uv_units']='1 UV unit equals 1 m; per-map repeat multiplies this metric UV'
(OUT/'manifest_v04.json').write_text(json.dumps(metadata,ensure_ascii=False,indent=2),encoding='utf-8')
(ART/'manifest.json').write_text(json.dumps(metadata,ensure_ascii=False,indent=2),encoding='utf-8')

# Context shell is linked from original source only after exports, never delivered twice.
with bpy.data.libraries.load(str(CONTEXT/'Art/Explorer/V2/STAR_Explorer_V2.blend'),link=False) as (src,dst):
...
with bpy.data.libraries.load(str(CONTEXT/'Art/Explorer/CockpitV3/STAR_CockpitV3SideDetails.blend'),link=False) as (src,dst):
 dst.objects=[n for n in src.objects if n == 'SM_CockpitV3SideDetails']
for o in dst.objects:
 if o:bpy.context.collection.objects.link(o);o['context_only']=True
...
print('COCKPIT_V4_DONE',sum(p['triangles'] for p in manifest),flush=True)
