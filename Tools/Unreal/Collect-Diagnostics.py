...
summary={'state':'CLEAN_EXIT_RECORDED' if clean else 'NO_CLEAN_EXIT_RECORD',
    'interpretation':'正常終了処理の開始を記録。終了完了はエンジンログも確認してください。' if clean else '正常終了記録なし。実行中・強制終了・クラッシュの区別は追加確認が必要です。',
...
summary['longestGameThreadOperations']=sorted((r for r in rows if 'milliseconds' in r and r.get('frame',-1)>=0 and r['type'] in ('operation_end','game_thread_hitch')),key=lambda r:r['milliseconds'],reverse=True)[:15]
...
with zipfile.ZipFile(out/'diagnostics.zip','x',zipfile.ZIP_DEFLATED) as archive:
    ...
    log=a.engine_log.resolve() if a.engine_log else saved/'Logs/Star.log'
